...
def certificate_pdf(sid,name,course):
    ...
    title = ParagraphStyle(name="t", fontSize=30, alignment=TA_CENTER)
    ...
    content.append(Paragraph(f"Completed <b>{course}</b>", styles["Normal"]))
...
# ================= MAIN =================
    ...
    if role=="Faculty":
        ...
        if ch=="My Schedule":
            ...
            if my.empty:
                st.warning("No schedule assigned yet")
            else:
                st.dataframe(my)
