# ================= CAMPUS ERP FINAL FIXED VERSION =================
...
def report_pdf(sid,name,course,df):
    ...
    layout = Table([[Image(qr,2*inch,2*inch), info_data]])
...
def certificate_pdf(sid,name,course):
    ...
    content.append(Spacer(1,10))
    content.append(Paragraph(f"<b>{name}</b>", para))
    content.append(Spacer(1,15))
    content.append(Paragraph(f"has successfully completed the course {course}", para))

    content.append(Spacer(1,25))

    qr = make_qr(f"CERT-{sid}-{name}", f"{sid}_certqr.png")
    layout = Table([[Image(qr,2.2*inch,2.2*inch), Paragraph(f"Student ID: {sid}", styles["Normal"])]])
